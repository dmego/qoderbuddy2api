...
const metadata: Record<string, Meta> = {
  ...
  "growth.auto_tasks": { label: "自动接受/领取任务", description: "签到后自动接受未接受的任务并领取可领奖的奖励。" },
  "growth.auto_lottery": { label: "自动抽奖", description: "有抽奖次数时自动抽，实物奖品留在记录里不发货。" },
  "growth.auto_travel": { label: "自动派 Buddy 旅行", description: "能量充足时自动出发，旅行完成自动领积分。" },
  ...
  "growth.auto_active_day": { label: "登录自动化（每日活跃日）", description: "与签到解耦，经成长调度器每天自动执行一次正式 WorkBuddy 对话点亮当天；启动后自动补跑未执行的天，失败可手动重试。" },
...
function groupLabel(group: string): string { return ({ service: "服务", checkin: "签到", monitoring: "指标", usage: "用量", growth: "Growth 自动化" } as Record<string, string>)[group] ?? group; }
</script>
