...
// A fixed threshold cannot describe every provider: a 2-hour OrcaTerm token is
// always "within 7 days", so a freshly rotated credential would still read as
// about to lapse. Compare what is left against the credential's own lifetime
// instead, which reads correctly for a 2-hour token and a 45-day one alike.
function expiryState(item: Credential): string {
  if (!item.expires_at) return "no_expiry";
  const remaining = new Date(item.expires_at).valueOf() - Date.now();
  if (remaining <= 0) return "expired";
  const issued = new Date(item.updated_at).valueOf();
  const lifetime = new Date(item.expires_at).valueOf() - issued;
  if (!Number.isFinite(lifetime) || lifetime <= 0) return "valid";
  return remaining / lifetime < 0.1 ? "expiring" : "valid";
}
function refreshLabel(item: Credential): string {
  // The backend flag already means "renewable", which for OrcaTerm is true even
  // though it carries no refresh_token field.
  return item.has_refresh_token ? "可刷新" : "无 refresh";
}
