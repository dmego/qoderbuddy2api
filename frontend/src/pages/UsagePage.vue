...
<template>
  <section class="page-content">
    ...
    <div class="summary-grid summary-grid--five" :aria-busy="summary.isPending.value"><article class="summary-tile"><Activity :size="18" /><span>请求总数</span><strong>{{ number(summary.data.value?.summary.request_count) }}</strong><small>成功 {{ number(summary.data.value?.summary.success_count) }}</small></article><article class="summary-tile"><BarChart3 :size="18" /><span>输入 Token</span><strong>{{ summary.data.value?.summary.token_event_count ? number(summary.data.value.summary.input_tokens) : "不可用" }}</strong><small>实际用量事件</small></article><article class="summary-tile"><BarChart3 :size="18" /><span>输出 Token</span><strong>{{ summary.data.value?.summary.token_event_count ? number(summary.data.value.summary.output_tokens) : "不可用" }}</strong><small>缺失 {{ number(summary.data.value?.summary.missing_token_count) }} 个事件</small></article><article class="summary-tile"><CalendarClock :size="18" /><span>延迟</span><strong>{{ summary.data.value?.summary.latency_p95_ms != null ? `${summary.data.value.summary.latency_p95_ms} ms` : "--" }}</strong><small>平均 {{ summary.data.value?.summary.latency_avg_ms != null ? `${summary.data.value.summary.latency_avg_ms} ms` : "--" }}</small></article><article class="summary-tile"><CalendarClock :size="18" /><span>错误率</span><strong>{{ summary.data.value ? `${Math.round(summary.data.value.summary.error_count / Math.max(summary.data.value.summary.request_count, 1) * 100)}%` : "--" }}</strong><small><StatePill :value="summary.data.value?.summary.status ?? (summary.isStale.value ? 'stale' : 'fresh')" /></small></article></div>
    ...
    <section class="data-panel"><PanelHeader title="请求事件" :description="`第 ${page} 页 · 详情仅含脱敏状态元数据`" /><PaginatedTable aria-label="请求事件" :loading="events.isPending.value" :error="events.isError.value ? `事件读取失败：${events.error.value}` : ''" :empty="!(events.data.value?.events.length)" empty-title="暂无匹配事件" empty-description="放宽筛选时间或检查代理服务是否已产生请求。" :stale="events.isStale.value" :page="page" :total="events.data.value?.total" :page-size="25" :can-previous="canPrevious.length > 0" :can-next="Boolean(events.data.value?.next_cursor)" @retry="events.refetch()" @previous="previous" @next="next(events.data.value?.next_cursor)"><template #header><tr><th>时间</th><th>服务提供方 / 账号</th><th>模型</th><th>协议</th><th>状态</th><th>Token</th><th>延迟</th></tr></template><tr v-for="event in events.data.value?.events ?? []" :key="event.event_id" :class="{ selected: selectedEventId === event.event_id }"><td><button class="table-link" :data-testid="`usage-event-${event.event_id}`" type="button" @click="selectedEventId = event.event_id"><strong>{{ event.started_at }}</strong><small>{{ event.event_id }}</small></button></td><td>{{ event.provider }}<small>{{ event.account_id ?? "账号未知" }}</small></td><td class="mono">{{ event.model_id }}</td><td>{{ event.protocol }}</td><td><StatePill :value="event.status" /><small>HTTP {{ event.http_status ?? "--" }}</small></td><td>{{ tokenLabel(event) }}</td><td>{{ event.latency_ms == null ? "--" : `${event.latency_ms} ms` }}</td></tr></PaginatedTable></section>

    <OperationStatus :operation="lastOperation" />

    <AccessibleDrawer :open="Boolean(selectedEventId)" title="事件详情" :subtitle="selectedEventId" close-label="关闭事件详情" @close="selectedEventId = ''"><div v-if="detail.isPending.value" class="loading-row">正在读取详情…</div><div v-else-if="detail.isError.value" class="data-state data-state--error">事件详情读取失败。<button class="secondary-button compact-button" type="button" @click="detail.refetch()">重试</button></div><dl v-else-if="detail.data.value" class="detail-list"><div><dt>请求 ID</dt><dd class="mono">{{ detail.data.value.request_id }}</dd></div><div><dt>服务提供方 / 账号</dt><dd>{{ detail.data.value.provider }} / {{ detail.data.value.account_id ?? "--" }}</dd></div><div><dt>模型 / 协议</dt><dd>{{ detail.data.value.model_id }} / {{ detail.data.value.protocol }}</dd></div><div><dt>状态</dt><dd><StatePill :value="detail.data.value.status" /> HTTP {{ detail.data.value.http_status ?? "--" }}</dd></div><div><dt>Token</dt><dd>{{ tokenLabel(detail.data.value) }}</dd></div><div><dt>流式提交</dt><dd>{{ streamCommittedLabel(detail.data.value.stream_committed) }}</dd></div><div><dt>错误代码</dt><dd>{{ detail.data.value.error_code ?? "--" }}</dd></div></dl></AccessibleDrawer>
