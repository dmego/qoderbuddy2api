...
<template>
  <section class="page-content">
    ...
    <section class="data-panel usage-filters"><PanelHeader title="筛选请求" description="所有条件同时作用于摘要、趋势、事件详情和 CSV 导出。"><button class="secondary-button compact-button" type="button" @click="resetFilters"><X :size="14" />清除</button></PanelHeader><div class="usage-filter-grid"><label>Provider<select v-model="provider" aria-label="Provider"><option value="">全部 Provider</option><option value="codebuddy">CodeBuddy</option><option value="qoder">Qoder</option></select></label><label>账号 ID<input v-model.trim="accountId" aria-label="账号 ID" placeholder="例如 qd-1" /></label><label>模型 ID<input v-model.trim="modelId" aria-label="模型 ID" placeholder="例如 model-a" /></label><label>状态<select v-model="statusFilter" aria-label="请求状态"><option value="">全部</option><option value="succeeded">成功</option><option value="failed">失败</option></select></label><label>开始时间<input v-model="startedAfter" aria-label="开始时间" type="datetime-local" /></label><label>结束时间<input v-model="startedBefore" aria-label="结束时间" type="datetime-local" /></label></div></section>
