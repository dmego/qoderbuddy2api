...
import { formatBeijing, formatDuration, formatTokens } from "@/utils/format";

type Summary = { request_count: number; input_tokens: number; output_tokens: number; success_count: number; error_count: number; cancelled_count?: number; token_event_count: number; missing_token_count: number; latency_avg_ms?: number | null; latency_p95_ms?: number | null; ttft_avg_ms?: number | null; ttft_p95_ms?: number | null; observed_at?: string; status?: string };
...
// 成功/已取消/失败 三分支：客户端主动放弃的请求既不算成功也不算失败。
function outcomeNote(summary?: Summary): string {
  if (!summary) return "";
  const parts = ["成功 " + number(summary.success_count)];
  if (summary.cancelled_count) parts.push("已取消 " + number(summary.cancelled_count));
  if (summary.error_count) parts.push("失败 " + number(summary.error_count));
  return parts.join(" · ");
}
