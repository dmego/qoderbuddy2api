...
<template>
  <section class="page-content">
    ...
    <section class="data-panel filter-panel">
      ...
      <div class="filter-grid filter-grid--six"><label class="filter-search">名称或账号 ID<div class="input-with-icon"><Search :size="15" /><input v-model="draftSearch" placeholder="回车搜索" @keyup.enter="applySearch" /></div></label><label>Provider<select v-model="provider" @change="reset"><option value="">全部</option><option value="codebuddy">CodeBuddy</option><option value="qoder">Qoder</option></select></label><label>来源<select v-model="source" @change="reset"><option value="">全部</option><option value="env">环境变量</option><option value="manual">手动导入</option><option value="oauth">OAuth</option><option value="import">导入</option></select></label><label>状态<select v-model="statusFilter" @change="reset"><option value="">全部</option><option value="active">正常</option><option value="action_required">需处理</option><option value="pending">待验证</option><option value="disabled">停用</option></select></label><label>用途<select v-model="purpose" @change="reset"><option value="">全部</option><option value="chat">Chat</option><option value="checkin">Check-in</option></select></label><div class="filter-actions"><button type="button" @click="applySearch"><Search :size="15" />应用</button><button class="secondary-button" type="button" @click="clearFilters"><X :size="15" />清除</button></div></div>
    </section>
