...
const accounts = useQuery({
  ...
  queryFn: () => apiRequest<AccountPage>(appendQuery("/accounts", { limit: 20, cursor: cursor.value, search: search.value, provider: provider.value, source: source.value, status: statusFilter.value, purpose: purpose.value })),
  staleTime: 15_000,
});
...
const batchAction = useMutation({
  mutationFn: async (kind: "refresh" | "probe" | "disable") => {
    ...
    const results = await Promise.allSettled(targets.map((item) => kind === "disable" ? disableAccount(item) : mutateAccount(item, kind)));
    return { action: `批量${accountActionLabel(kind)}`, status: results.some((item) => item.status === "rejected") ? "failed" : "succeeded", total: results.length, succeeded: results.filter((item) => item.status === "fulfilled").length };
  },
  onSuccess: async (result) => { lastOperation.value = result; notify("批量操作已结束", { message: `${result.succeeded} / ${result.total} 个账号成功`, tone: result.status === "failed" ? "warning" : "success" }); selection.value = []; await queryClient.invalidateQueries({ queryKey: ["accounts"] }); },
  onError: (error) => notify("批量操作失败", { message: String(error), tone: "error" }),
});

const allSelected = computed(() => currentAccounts().length > 0 && currentAccounts().every((item) => selection.value.includes(accountKey(item))));
...
function requestAction(account: Account, kind: "toggle" | "promote" | "delete" | "refresh" | "probe"): void { if (kind === "delete" || (kind === "toggle" && account.enabled)) pendingConfirm.value = { kind: kind === "toggle" ? "disable" : "delete", account }; else action.mutate({ account, kind }); }
function confirmPending(): void { const pending = pendingConfirm.value; pendingConfirm.value = null; if (!pending) return; if (pending.account) action.mutate({ account: pending.account, kind: pending.kind === "disable" ? "toggle" : "delete" }); else if (pending.kind !== "delete") batchAction.mutate(pending.kind); }
function requestBatch(kind: "refresh" | "probe" | "disable"): void { if (selection.value.length) pendingConfirm.value = { kind }; }
...
function toggleAll(): void { selection.value = allSelected.value ? [] : currentAccounts().map(accountKey); }
...
<template>
  <section class="page-content">
    <header class="page-header"><div><p class="eyebrow">Identity & access</p><h1>账号管理</h1><p>按 Provider、来源、状态和用途管理账号池，并对刷新与探测操作提供可追踪反馈。</p></div><button type="button" @click="showImport = !showImport"><Plus :size="16" />{{ showImport ? "收起导入" : "添加账号" }}</button></header>
    ...
    <section class="data-panel filter-panel">
      ...
      <div class="filter-grid filter-grid--six"><label class="filter-search">名称或账号 ID<div class="input-with-icon"><Search :size="15" /><input v-model="draftSearch" placeholder="回车搜索" @keyup.enter="applySearch" /></div></label><label>Provider<select v-model="provider" @change="reset"><option value="">全部</option><option value="codebuddy">CodeBuddy</option><option value="qoder">Qoder</option><option value="workbuddy">WorkBuddy</option></select></label><label>来源<select v-model="source" @change="reset"><option value="">全部</option><option value="env">环境变量</option><option value="manual">手动导入</option><option value="oauth">OAuth</option></select></label><label>状态<select v-model="statusFilter" @change="reset"><option value="">全部</option><option value="active">正常</option><option value="needs_reauth">需重新授权</option><option value="disabled">停用</option></select></label><label>用途<select v-model="purpose" @change="reset"><option value="">全部</option><option value="chat">Chat</option><option value="checkin">Check-in</option></select></label><div class="filter-actions"><button type="button" @click="applySearch"><Search :size="15" />应用</button><button class="secondary-button" type="button" @click="clearFilters"><X :size="15" />清除</button></div></div>
    </section>

    <section class="data-panel">
      <PanelHeader title="账号池" :description="`第 ${page} 页 · 环境变量账号只读`"><div class="toolbar"><span v-if="selection.length" class="selection-count">已选 {{ selection.length }}</span><button class="secondary-button compact-button" type="button" :disabled="!selection.length || batchAction.isPending.value" @click="requestBatch('refresh')"><RefreshCcw :size="14" />批量刷新</button><button class="secondary-button compact-button" type="button" :disabled="!selection.length || batchAction.isPending.value" @click="requestBatch('probe')"><Activity :size="14" />批量探测</button><button class="danger-button compact-button" type="button" :disabled="!selection.length || batchAction.isPending.value" @click="requestBatch('disable')">批量停用</button></div></PanelHeader>
      <PaginatedTable aria-label="账号池" :loading="accounts.isPending.value" :error="accounts.isError.value ? `账号读取失败：${accounts.error.value}` : ''" :empty="!currentAccounts().length" empty-title="没有匹配的账号" empty-description="调整筛选条件，或通过上方导入账号。" :stale="accounts.isStale.value" :unavailable="metrics.isError.value" :page="page" :total="accounts.data.value?.total" :can-previous="canPrevious.length > 0" :can-next="Boolean(accounts.data.value?.next_cursor)" @retry="accounts.refetch()" @previous="previous" @next="next(accounts.data.value?.next_cursor)">
        <template #header><tr><th><input type="checkbox" aria-label="选择当前页全部账号" :checked="allSelected" @change="toggleAll" /></th><th>账号</th><th>Provider</th><th>用途</th><th>来源</th><th>指标</th><th>状态</th><th>操作</th></tr></template>
        <tr v-for="account in currentAccounts()" :key="accountKey(account)" :class="{ selected: selected && accountKey(selected) === accountKey(account) }"><td><input type="checkbox" :aria-label="`选择 ${account.label}`" :checked="selection.includes(accountKey(account))" @change="toggleSelection(accountKey(account))" /></td><td><button class="table-link" type="button" @click="selected = account"><strong>{{ account.label }}</strong><small>{{ account.account_id }} · {{ account.masked_identity ?? "无身份掩码" }}</small></button></td><td><span class="provider-mark" :class="`provider-mark--${account.provider}`">{{ account.provider }}</span></td><td><div class="purpose-list"><span v-for="(item, name) in account.purposes" :key="name" class="purpose-chip"><b>{{ name }}</b><StatePill :value="item.status" /></span><span v-if="!Object.keys(account.purposes).length">未配置</span></div></td><td>{{ account.source }}<span v-if="account.shadowed" class="text-warning"> · 被覆盖</span></td><td><StatePill :value="metricStatus(account)" /></td><td><StatePill :value="account.enabled ? account.summary_status : 'disabled'" /></td><td><div class="row-actions"><button class="icon-button" type="button" :aria-label="`刷新 ${account.label}`" @click="requestAction(account, 'refresh')"><RefreshCcw :size="15" /></button><button class="icon-button" type="button" :aria-label="`探测 ${account.label}`" @click="requestAction(account, 'probe')"><Activity :size="15" /></button><button class="icon-button" type="button" :aria-label="account.enabled ? `停用 ${account.label}` : `启用 ${account.label}`" @click="requestAction(account, 'toggle')"><ShieldCheck :size="15" /></button><button v-if="account.source === 'env'" class="icon-button" type="button" :aria-label="`提升 ${account.label}`" @click="requestAction(account, 'promote')"><ChevronRight :size="16" /></button><button v-else class="icon-button danger-icon" type="button" :aria-label="`删除 ${account.label}`" @click="requestAction(account, 'delete')"><Trash2 :size="15" /></button></div></td></tr>
      </PaginatedTable>
    </section>

    <OperationStatus :operation="lastOperation" />

    <aside v-if="selected" class="detail-drawer data-panel" aria-label="账号详情"><div class="drawer-heading"><div><p class="eyebrow">Account detail</p><h2>{{ selected.label }}</h2><p>{{ selected.provider }} / {{ selected.account_id }}</p></div><button class="icon-button" type="button" aria-label="关闭账号详情" @click="selected = null"><X :size="16" /></button></div><dl class="detail-list"><div><dt>来源</dt><dd>{{ selected.source }}</dd></div><div><dt>身份掩码</dt><dd>{{ selected.masked_identity ?? "--" }}</dd></div><div><dt>总体状态</dt><dd><StatePill :value="selected.summary_status" /></dd></div></dl><h3>用途状态</h3><div class="purpose-cards"><div v-for="(item, name) in selected.purposes" :key="name"><strong>{{ name }}</strong><StatePill :value="item.status" /><small>{{ item.verification_status }}<template v-if="item.expires_at"> · 到期 {{ item.expires_at }}</template></small></div></div><div class="drawer-subhead"><h3>Token、配额与签到</h3><button class="icon-button" type="button" aria-label="刷新账号指标" @click="selectedMetrics.refetch()"><RefreshCcw :size="15" /></button></div><div v-if="selectedMetrics.isPending.value" class="loading-row">正在读取账号指标…</div><div v-else-if="selectedMetrics.isError.value" class="data-state data-state--warning">详细指标暂不可用，可稍后重试。</div><div v-else-if="detailMetrics.length" class="metric-list"><div v-for="metric in detailMetrics" :key="metric.metric_kind"><strong>{{ metric.metric_kind }}</strong><StatePill :value="metric.status" /><span>{{ metricSummary(metric) }}</span><small>采集于 {{ metric.observed_at ?? "--" }}</small></div></div><p v-else class="compact-empty">尚未采集账号指标。</p><div class="form-actions"><button type="button" :disabled="action.isPending.value" @click="requestAction(selected, 'refresh')"><RefreshCcw :size="15" />刷新账号</button><button class="secondary-button" type="button" :disabled="action.isPending.value" @click="requestAction(selected, 'probe')"><Activity :size="15" />安全探测</button></div></aside>

    <ConfirmDialog :open="Boolean(pendingConfirm)" :title="pendingConfirm?.account ? `${pendingConfirm.kind === 'delete' ? '删除' : '停用'} ${pendingConfirm.account.label}？` : `批量${accountActionLabel(pendingConfirm?.kind ?? '')}？`" :description="pendingConfirm?.kind === 'delete' ? '这会撤销持久化账号及其代理与签到用途，操作不可撤销。' : pendingConfirm?.account ? '停用后该账号不会再参与新的代理或签到调度。' : `将对当前选中的 ${selection.length} 个账号逐一执行，失败不会阻止其他账号。`" :confirm-label="pendingConfirm?.kind === 'delete' ? '确认删除' : '确认执行'" :tone="pendingConfirm?.kind === 'delete' || pendingConfirm?.kind === 'disable' ? 'danger' : 'default'" :busy="action.isPending.value || batchAction.isPending.value" @cancel="pendingConfirm = null" @confirm="confirmPending" />
