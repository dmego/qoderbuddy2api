...
import { ClipboardCheck, DatabaseBackup, Eye, Filter, History, RefreshCcw, ShieldCheck, X } from "@lucide/vue";
...
const audit = useQuery({ queryKey: ["audit", cursor, search, actionFilter, resourceType, resultFilter], queryFn: () => apiRequest<AuditPage>(appendQuery("/audit", { limit: 25, cursor: cursor.value, search: search.value, action: actionFilter.value, resource_type: resourceType.value, result: resultFilter.value })), staleTime: 30_000 });
...
function clearFilters(): void { search.value = ""; actionFilter.value = ""; resourceType.value = ""; resultFilter.value = ""; reset(); }
...
<template>
  <section class="page-content">
    ...
    <section class="data-panel filter-panel">
      <PanelHeader title="管理审计" :description="`第 ${page} 页 · 只展示操作元数据`"><Filter :size="17" /></PanelHeader><div class="filter-grid filter-grid--five"><label class="filter-search">搜索<input v-model.trim="search" placeholder="事件、动作或资源 ID" @change="reset" /></label><label>动作<select v-model="actionFilter" @change="reset"><option value="">全部</option><option value="account">账号</option><option value="credential">凭据</option><option value="settings">设置</option><option value="backup">备份</option><option value="service">服务</option></select></label><label>资源<select v-model="resourceType" @change="reset"><option value="">全部</option><option value="account">Account</option><option value="credential">Credential</option><option value="setting">Setting</option><option value="backup">Backup</option><option value="service">Service</option></select></label><label>结果<select v-model="resultFilter" @change="reset"><option value="">全部</option><option value="succeeded">成功</option><option value="failed">失败</option><option value="rejected">拒绝</option></select></label><div class="filter-actions"><button class="secondary-button" type="button" @click="clearFilters"><X :size="15" />清除</button></div></div>
    ...
    <aside v-if="selectedBackup" class="detail-drawer data-panel" aria-label="备份详情"><div class="drawer-heading"><div><p class="eyebrow">Backup detail</p><h2>备份详情</h2><p class="mono">{{ selectedBackup.backup_id }}</p></div><button class="icon-button" type="button" aria-label="关闭备份详情" @click="selectedBackup = null"><X :size="16" /></button></div><dl class="detail-list"><div><dt>状态</dt><dd><StatePill :value="selectedBackup.status" /></dd></div><div><dt>Schema</dt><dd>v{{ selectedBackup.schema_version }}</dd></div><div><dt>大小</dt><dd>{{ size(selectedBackup.size_bytes) }}</dd></div><div><dt>SHA-256</dt><dd class="mono">{{ selectedBackup.sha256 ?? "--" }}</dd></div><div><dt>完成时间</dt><dd>{{ selectedBackup.finished_at ?? "--" }}</dd></div><div><dt>错误</dt><dd :class="{ 'text-danger': selectedBackup.error_message }">{{ selectedBackup.error_message ?? "无" }}</dd></div></dl></aside>
