...
type BackupPage = { backups: Backup[]; next_cursor?: string | null; total?: number };
...
const { cursor: backupCursor, page: backupPage, canPrevious: backupCanPrevious, next: nextBackup, previous: previousBackup, reset: resetBackups } = useCursorPager();
...
const backups = useQuery({ queryKey: ["backups", backupCursor, backupStatus], queryFn: () => apiRequest<BackupPage>(appendQuery("/backup", { limit: backupPageSize, cursor: backupCursor.value, status: backupStatus.value })), staleTime: 30_000 });
const backupCount = computed(() => backups.data.value?.total ?? 0);
...
function updateBackupStatus(event: Event): void { resetBackups(); backupStatus.value = selectValue(event); }
...
<template>
  <section class="page-content">
    <header class="page-header"><div><h1>审计与备份</h1><p>追踪管理变更，创建 SQLite 快照，并在离线恢复前执行校验和、结构与完整性预检。</p></div><button type="button" :disabled="operation.isPending.value" @click="pendingAction = { kind: 'create' }"><DatabaseBackup :size="16" />创建 SQLite 备份</button></header>
    <div class="security-banner"><ShieldCheck :size="18" /><div><strong>恢复保护</strong><span>管理台只调用预检校验，不会在线覆盖数据库；凭据主密钥和 .env 不包含在 SQLite 备份中。</span></div></div>

    <section class="data-panel"><PanelHeader title="备份快照" :description="`${backupCount} 个快照 · 第 ${backupPage} 页`"><div class="toolbar"><select :value="backupStatus" aria-label="筛选备份状态" @change="updateBackupStatus"><option value="">全部状态</option><option value="succeeded">成功</option><option value="failed">失败</option><option value="cancelled">已取消</option></select><button class="icon-button" type="button" aria-label="刷新备份列表" title="刷新备份列表" @click="backups.refetch()"><RefreshCcw :size="16" /></button></div></PanelHeader><PaginatedTable aria-label="备份快照" :loading="backups.isPending.value" :error="backups.isError.value ? `备份列表读取失败：${backups.error.value}` : ''" :empty="!(backups.data.value?.backups.length)" empty-title="还没有备份快照" empty-description="创建备份后，可在这里核对校验和与结构版本。" :stale="backups.isStale.value" :page="backupPage" :page-size="backupPageSize" :total="backupCount" :can-previous="backupCanPrevious.length > 0" :can-next="Boolean(backups.data.value?.next_cursor)" @retry="backups.refetch()" @previous="previousBackup" @next="nextBackup(backups.data.value?.next_cursor)"><template #header><tr><th>备份 ID / 时间</th><th>状态</th><th>结构版本</th><th>大小</th><th>SHA-256</th><th>操作</th></tr></template><tr v-for="item in backups.data.value?.backups ?? []" :key="item.backup_id" :class="{ selected: selectedBackup?.backup_id === item.backup_id }"><td><strong class="mono">{{ item.backup_id }}</strong><small>{{ item.started_at }}</small></td><td><StatePill :value="item.status" /><small v-if="item.error_message" class="text-danger">{{ item.error_message }}</small></td><td>v{{ item.schema_version }}</td><td>{{ size(item.size_bytes) }}</td><td class="mono hash-cell" :title="item.sha256">{{ item.sha256 ?? "--" }}</td><td><div class="row-actions"><button class="icon-button" type="button" :aria-label="`查看备份 ${item.backup_id}`" :title="`查看备份 ${item.backup_id}`" @click="selectedBackup = item"><Eye :size="15" /></button><button class="secondary-button compact-button" type="button" :disabled="operation.isPending.value || item.status !== 'succeeded'" @click="pendingAction = { kind: 'validate', backup: item }"><ClipboardCheck :size="15" />校验恢复</button></div></td></tr></PaginatedTable></section>

    <section class="data-panel filter-panel">
      <PanelHeader title="管理审计" :description="`第 ${page} 页 · 只展示操作元数据`"><Filter :size="17" /></PanelHeader><div class="filter-grid filter-grid--six"><label class="filter-search">搜索<input v-model="draftSearch" aria-label="审计搜索" placeholder="事件、动作或资源 ID" @keyup.enter="applyTextFilters" /></label><label>动作前缀<input v-model="draftActionPrefix" aria-label="动作前缀" placeholder="例如 account" @keyup.enter="applyTextFilters" /></label><label>类别<select :value="category" aria-label="审计类别" @change="updateCategory"><option value="">全部</option><option value="account">账号</option><option value="backup">备份</option><option value="checkin">签到</option><option value="credential">凭据</option><option value="metrics">指标</option><option value="model">模型</option><option value="proxy_key">代理密钥</option><option value="service">服务</option><option value="settings">设置</option><option value="usage">用量</option></select></label><label>资源<select :value="resourceType" aria-label="审计资源" @change="updateResourceType"><option value="">全部</option><option value="account">账号</option><option value="backup">备份</option><option value="checkin">签到</option><option value="credential">凭据</option><option value="metrics">指标</option><option value="model">模型</option><option value="proxy_key">代理密钥</option><option value="service">服务</option><option value="setting">设置项</option><option value="usage">用量</option></select></label><label>结果<select :value="resultFilter" aria-label="审计结果" @change="updateResult"><option value="">全部</option><option value="succeeded">成功</option><option value="failed">失败</option><option value="running">运行中</option><option value="cancelled">已取消</option></select></label><div class="filter-actions"><button type="button" @click="applyTextFilters"><Search :size="15" />应用</button><button class="secondary-button" type="button" @click="clearFilters"><X :size="15" />清除</button></div></div>
      <PaginatedTable aria-label="管理审计事件" :loading="audit.isPending.value" :error="audit.isError.value ? `审计读取失败：${audit.error.value}` : ''" :empty="!(audit.data.value?.events.length)" empty-title="暂无匹配的审计记录" empty-description="管理变更发生后，安全元数据会持久化到这里。" :stale="audit.isStale.value" :page="page" :page-size="25" :total="audit.data.value?.total" :can-previous="canPrevious.length > 0" :can-next="Boolean(audit.data.value?.next_cursor)" @retry="audit.refetch()" @previous="previous" @next="next(audit.data.value?.next_cursor)"><template #header><tr><th>时间</th><th>动作</th><th>资源</th><th>操作主体</th><th>结果</th><th>事件 ID</th></tr></template><tr v-for="event in audit.data.value?.events ?? []" :key="event.event_id"><td>{{ event.created_at }}</td><td><History :size="14" /> {{ event.action }}</td><td>{{ event.resource_type }}<small>{{ event.resource_id ?? "--" }}</small></td><td>{{ event.actor_type ?? "管理员" }}</td><td><StatePill :value="event.result" /><small v-if="event.error_code" class="text-danger">{{ event.error_code }}</small></td><td class="mono">{{ event.event_id }}</td></tr></PaginatedTable>
    </section>

    <OperationStatus :operation="lastOperation" />
    <AccessibleDrawer :open="Boolean(selectedBackup)" title="备份详情" :subtitle="selectedBackup?.backup_id ?? ''" close-label="关闭备份详情" @close="selectedBackup = null"><template v-if="selectedBackup"><dl class="detail-list"><div><dt>状态</dt><dd><StatePill :value="selectedBackup.status" /></dd></div><div><dt>结构版本</dt><dd>v{{ selectedBackup.schema_version }}</dd></div><div><dt>大小</dt><dd>{{ size(selectedBackup.size_bytes) }}</dd></div><div><dt>SHA-256</dt><dd class="mono">{{ selectedBackup.sha256 ?? "--" }}</dd></div><div><dt>完成时间</dt><dd>{{ selectedBackup.finished_at ?? "--" }}</dd></div><div><dt>错误</dt><dd :class="{ 'text-danger': selectedBackup.error_message }">{{ selectedBackup.error_message ?? "无" }}</dd></div></dl></template></AccessibleDrawer>
