...
const history = useQuery({
  queryKey: ["account-growth-history", provider, accountId],
  enabled: computed(() => Boolean(accountId.value)),
  queryFn: () => apiRequest<{ logs: HistoryEntry[] }>(`${base.value}/growth/history?limit=20`),
  staleTime: 15_000,
});
...
const lastResults = computed(() => {
  const logs = history.data.value?.logs ?? [];
...
function stepLabel(key: StepKey): string {
  return { tasks: "任务", lottery: "抽奖", travel: "旅行", redeem: "兑换", buddy_open: "Buddy" }[key];
}
...
function historySummary(entry: HistoryEntry): string {
  return Object.entries(entry.results).map(([, v]) => {
    if (!v) return "";
    const icon = v.status === "completed" ? "✅" : v.status === "skipped" || v.status === "no_chances" || v.status === "daily_limit_reached" || v.status === "insufficient" ? "⏭️" : "⚠️";
  ...
  <section class="page-content growth-page">
    ...
    <template v-else-if="accountId">
      ...
      <template v-else-if="growth.data.value">
            ...
            <div v-for="step in stepConfigs" :key="step.key" class="automation-card" :class="{ 'automation-card--off': !step.enabled }">
              ...
              <div class="automation-card-body">
                ...
                <div class="automation-meta">
                  <small v-if="findLatestStepLog(step.key)">上次: {{ findLatestStepLog(step.key)!.created_at.slice(5, 16).replace('T', ' ') }} · {{ triggerLabel(findLatestStepLog(step.key)!.triggered_by) }}</small>
                  <small v-else>尚未执行</small>
                </div>
              </div>
              <div class="automation-card-footer">
                <button class="secondary-button compact-button" type="button" :disabled="runningStep !== null" @click="runStep.mutate(step.key)">
                  <Play :size="14" />{{ runningStep === step.key ? '执行中…' : '手动执行' }}
                </button>
              </div>
            </div>
          </div>
        ...
        <!-- 自动化执行历史 -->
        <section class="data-panel">
          <PanelHeader title="执行历史" description="最近自动化执行记录。" />
          ...
          <div v-else class="history-list">
            <div v-for="entry in history.data.value.logs" :key="entry.id" class="history-entry">
              <div class="history-time">
                <small>{{ entry.created_at.slice(5, 16).replace('T', ' ') }}</small>
                <span class="history-trigger">{{ triggerLabel(entry.triggered_by) }}</span>
              </div>
              <div class="history-summary">{{ historySummary(entry) }}</div>
            </div>
          </div>
        </section>
      </template>
