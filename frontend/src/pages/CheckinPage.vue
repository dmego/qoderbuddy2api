...
function dailyOutcome(account: Account): string {
  ...
  return state?.terminal_outcome ?? "pending";
}
...
function attemptStatus(attempt: CheckinAttempt): string {
  ...
  if (attempt.outcome === "needs_reauth") {
    return "签到凭据已失效，请重新导入";
  }
