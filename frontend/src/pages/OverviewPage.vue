...
<template>
  <section class="page-content">
    ...
    <div class="overview-grid">
      ...
      <section class="data-panel"><PanelHeader title="账号池状态" description="可用账号与异常状态" /><ul class="health-list"><li><span>CodeBuddy</span><strong>{{ accounts.data.value?.accounts?.filter((item) => item.provider === 'codebuddy' && item.enabled).length ?? 0 }} 可用</strong></li><li><span>WorkBuddy 国际版</span><strong>{{ accounts.data.value?.accounts?.filter((item) => item.provider === 'workbuddy_intl' && item.enabled).length ?? 0 }} 可用</strong></li><li><span>OrcaTerm</span><strong>{{ accounts.data.value?.accounts?.filter((item) => item.provider === 'orcaterm' && item.enabled).length ?? 0 }} 可用</strong></li><li><span>Qoder</span><strong>{{ accounts.data.value?.accounts?.filter((item) => item.provider === 'qoder' && item.enabled).length ?? 0 }} 可用</strong></li></ul></section>
