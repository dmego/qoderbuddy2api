...
const summary = computed(() => [
  ...
  { label: "启用模型", value: String((models.data.value?.models ?? []).filter((item) => item.enabled).length), note: service.data.value?.observed_state ?? "Worker 未连接", icon: Cpu },
  ...
  { label: "Token", value: usage.data.value?.summary?.token_event_count ? String(((usage.data.value.summary.input_tokens ?? 0) + (usage.data.value.summary.output_tokens ?? 0)).toLocaleString()) : "不可用", note: "只统计真实 usage", icon: Coins },
]);
...
<template>
  <section class="page-content">
    <header class="page-header"><div><p class="eyebrow">Operations overview</p><h1>运行总览</h1><p>代理、账号、模型、Token、积分和签到的统一运行视图。</p></div><button class="secondary-button" type="button" @click="refreshAll"><RefreshCcw :size="16" />刷新全部</button></header>
