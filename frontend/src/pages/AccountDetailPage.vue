...
import { Activity, ArrowLeft, BadgeCheck, KeyRound, RefreshCcw, ShieldCheck, Trash2 } from "@lucide/vue";
...
type DetailAction = "save" | "refresh" | "probe" | "verify" | "promote" | "delete";
...
const action = useMutation({
  ...
  onSuccess: async (result, kind) => {
    lastOperation.value = { action: actionLabel(kind), status: "succeeded", ...(result as Record<string, unknown>) };
...
function executeAction(kind: DetailAction): Promise<unknown> {
  ...
  if (kind === "verify") return apiRequest(`${base.value}/verify-checkin`, { method: "POST" });
...
<template>
  <section class="page-content">
    ...
    <template v-else-if="account.data.value">
      <div v-if="isEnv" class="security-banner"><ShieldCheck :size="18" /><div><strong>环境变量账号</strong><span>此账号仅兼容既有 Chat 配置。请先提升为持久账号，才能编辑凭据或关联签到用途。</span></div></div>
      <section class="overview-grid"><div class="data-panel"><PanelHeader title="账号概览" description="只显示安全的身份掩码与用途状态。" /><dl class="detail-list"><div><dt>来源</dt><dd>{{ account.data.value.source }}</dd></div><div><dt>身份</dt><dd>{{ account.data.value.masked_identity ?? "--" }}</dd></div><div><dt>总体状态</dt><dd><StatePill :value="account.data.value.summary_status" /></dd></div><div><dt>最近更新</dt><dd>{{ account.data.value.updated_at ?? "--" }}</dd></div></dl></div><div class="data-panel"><PanelHeader title="账户操作" description="危险操作会要求确认。" /><div class="form-actions"><button type="button" :disabled="action.isPending.value" @click="requestAction('refresh')"><RefreshCcw :size="16" />刷新</button><button class="secondary-button" type="button" :disabled="action.isPending.value" @click="requestAction('probe')"><Activity :size="16" />探测</button><button class="secondary-button" type="button" :disabled="action.isPending.value" @click="requestAction('verify')"><BadgeCheck :size="16" />验证签到</button><button v-if="isEnv" class="secondary-button" type="button" :disabled="action.isPending.value" @click="requestAction('promote')">提升为持久账号</button><button v-else class="secondary-button" type="button" :disabled="action.isPending.value" @click="reauthorize"><KeyRound :size="16" />重新授权</button><button v-if="!isEnv" class="danger-button" type="button" :disabled="action.isPending.value" @click="requestAction('delete')"><Trash2 :size="16" />删除账号</button></div></div></section>
      <section class="data-panel"><PanelHeader title="用途与路由状态" description="Chat 与 Check-in 可以独立启停，签到失败不会移出 Chat 池。" /><div class="form-grid"><label>显示名称<input v-model="draftLabel" :disabled="!canWrite" aria-label="账号显示名称" /></label><label class="inline-check"><input v-model="draftEnabled" type="checkbox" :disabled="!canWrite" />账号启用</label><label v-for="name in ['chat', 'checkin']" :key="name" class="inline-check"><input :checked="name === 'chat' ? draftChat : draftCheckin" type="checkbox" :disabled="!canWrite || !account.data.value.purposes[name]" @change="setPurpose(name, $event)" /><span>{{ name }}</span><StatePill v-if="account.data.value.purposes[name]" :value="account.data.value.purposes[name].status" /></label></div><div class="purpose-cards"><div v-for="(item, name) in account.data.value.purposes" :key="name"><strong>{{ name }}</strong><StatePill :value="item.verification_status" /><small>到期：{{ item.expires_at ?? "未设置" }} · 验证：{{ item.verified_at ?? "尚未验证" }}<template v-if="item.last_error"> · {{ item.last_error }}</template></small></div></div><div class="form-actions"><button type="button" :disabled="!canWrite" @click="requestAction('save')"><ShieldCheck :size="16" />保存账号设置</button></div></section>
      <section class="overview-grid"><div class="data-panel"><PanelHeader title="凭据元数据" description="模式、版本与过期时间；不显示原始凭据或 fingerprint。" /><div v-if="credentials.isPending.value" class="loading-row">正在读取凭据元数据…</div><div v-else-if="!credentials.data.value?.length" class="compact-empty">尚未保存持久凭据。</div><div v-else class="metric-list"><div v-for="item in credentials.data.value" :key="item.purpose"><strong>{{ item.purpose }} · {{ item.mode }}</strong><StatePill :value="item.has_refresh_token ? 'refresh' : 'static'" /><span>版本 v{{ item.credential_version }} · 到期 {{ item.expires_at ?? "未设置" }}</span><small>最后更新 {{ item.updated_at }}</small></div></div></div><div class="data-panel"><PanelHeader title="积分与配额" description="未知或过期值不会显示为 0。" /><div v-if="metrics.isPending.value" class="loading-row">正在读取账号指标…</div><div v-else-if="!metrics.data.value?.snapshots.length" class="compact-empty">尚未采集账号指标。</div><div v-else class="metric-list"><div v-for="metric in metrics.data.value.snapshots" :key="metric.metric_kind"><strong>{{ metric.metric_kind }}</strong><StatePill :value="metric.status" /><span>{{ metricValue(metric) }}</span><small>{{ metric.observed_at ?? "--" }}</small></div></div></div></section>
