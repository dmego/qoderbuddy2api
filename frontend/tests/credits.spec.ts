...
describe("CreditsPage", () => {
  ...
  it("renders current credits, aggregates history, and filters by provider", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.includes("/metrics/accounts/qoder/qd-1/history")) return response({ rows: [quotaHistory(daysAgo(2), 90), quotaHistory(daysAgo(1), 80)] });
      return response({});
    }));
  ...
  it("includes Qoder add-on quota in the current balance", async () => {
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url.includes("/metrics/accounts?")) return response({ snapshots: [{ provider: "qoder", account_id: "qd-1", metric_kind: "quota", status: "fresh", observed_at: daysAgo(1), value: { user_quota: { remaining: 0 }, add_on_quota: { remaining: 4948 } } }] });
...
function quotaMetric(accountId: string, remaining: number) { return { provider: "qoder", account_id: accountId, metric_kind: "quota", status: "fresh", observed_at: daysAgo(1), value: { user_quota: { remaining, unit: "credits" } } }; }
