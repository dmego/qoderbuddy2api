...
class Settings:
    ...
    def from_env(cls, env_file: str | None = None) -> Settings:
        ...
        codebuddy_checkin_enabled = _env_bool("CODEBUDDY_CHECKIN_ENABLED", False)
        qoder_checkin_enabled = _env_bool("QODER_CHECKIN_ENABLED", False)
        # Global flag; if unset, enable only when a provider check-in flag is on
        checkin_raw = os.getenv("CHECKIN_ENABLED")
        if checkin_raw is not None:
            checkin_enabled = checkin_raw.strip().lower() in {"1", "true", "yes", "on"}
        else:
            checkin_enabled = codebuddy_checkin_enabled or qoder_checkin_enabled
        ...
        return cls(
            ...
            admin_cookie_secure=os.getenv("QB2API_ADMIN_COOKIE_SECURE", "auto").strip().lower()
            or "auto",
            ...
            codebuddy_checkin_enabled=codebuddy_checkin_enabled,
            codebuddy_checkin_base=os.getenv(
                "CODEBUDDY_CHECKIN_BASE", "https://www.workbuddy.cn"
            ),
            codebuddy_checkin_status_path=os.getenv(
                "CODEBUDDY_CHECKIN_STATUS_PATH", "/billing/meter/checkin-status"
            ),
            codebuddy_checkin_status_method=os.getenv(
                "CODEBUDDY_CHECKIN_STATUS_METHOD", ""
            ).strip().upper(),
            codebuddy_checkin_claim_path=os.getenv(
                "CODEBUDDY_CHECKIN_CLAIM_PATH", "/billing/meter/daily-checkin"
            ),
            codebuddy_checkin_claim_method=os.getenv(
                "CODEBUDDY_CHECKIN_CLAIM_METHOD", "POST"
            )
            .strip()
            .upper()
            or "POST",
            qoder_checkin_enabled=qoder_checkin_enabled,
            qoder_checkin_base=os.getenv(
                "QODER_CHECKIN_BASE", "https://openapi.qoder.com.cn"
            ),
            qoder_checkin_status_path=os.getenv(
                "QODER_CHECKIN_STATUS_PATH", "/sash/api/v1/me/daily-check-in/status"
            ),
            qoder_checkin_claim_path=os.getenv(
                "QODER_CHECKIN_CLAIM_PATH", "/sash/api/v1/me/daily-check-in/claim"
            ),
            qoder_checkin_refresh_path=os.getenv(
            ...
            usage_rollup_interval_seconds=_env_int(
                "QB2API_USAGE_ROLLUP_INTERVAL_SECONDS", 60
            ),
            usage_detail_retention_days=_env_int(
